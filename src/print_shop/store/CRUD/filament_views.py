...
def add_filament(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            filament = form.save()
            messages.success(request, f'Filament {filament.Name} was created successfully')
...
def edit_filament(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            filament = form.save()
            messages.success(request, f'Filament {filament.Name} was updated successfully')
            return redirect('filament-list')
    else:
...
def delete_filament(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('filament-list')
    return render(request, 'filament/filament_confirm_delete.html', {'filament': filament})