...
def raw_materials_list(request):
    ...
    return render(request, 'raw_materials/raw_materials_list.html', {'raw_materials': raw_materials})
...
def add_raw_material(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Raw Material {material_name} was created successfully')
...
def edit_raw_material(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            raw_material = form.save()
            material_name = raw_material.BrandName or raw_material.Filament.Name
            messages.success(request, f'Raw Material {material_name} was updated successfully')
            return redirect('raw-materials-list')
    else:
...
def delete_raw_material(request, pk):
    ...
    if request.method == 'POST':
        material_name = raw_material.BrandName or raw_material.Filament.Name
        raw_material.delete()
        messages.success(request, f'Raw Material {material_name} was deleted successfully')
        return redirect('raw-materials-list')
    return render(request, 'raw_materials/raw_material_confirm_delete.html', {'raw_material': raw_material})