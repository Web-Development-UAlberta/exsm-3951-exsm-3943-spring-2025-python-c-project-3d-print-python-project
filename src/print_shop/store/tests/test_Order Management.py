...
from store.models import Orders, OrderItems, InventoryChange, Materials, Filament, RawMaterials, Suppliers, Shipping
...
class OrderManagementTestCase(TestCase):
    def setUp(self):
        ...
        self.filament = Filament.objects.create(Name="PLA Blue", Material=self.material, ColorHexCode="0000FF")
        self.supplier = Suppliers.objects.create(Name="Test Supplier", Address="123 St", Phone="12345", Email="a@test.com")
        ...
        self.order = Orders.objects.create(User=self.user, Shipping=self.shipping, TotalPrice=50.00)
    ...
    def test_search_order_id(self):
        response = self.client.get(reverse("order_management"), {"search": str(self.order.id)})
